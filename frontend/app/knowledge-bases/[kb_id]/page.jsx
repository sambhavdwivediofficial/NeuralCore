// app/knowledge-bases/[kb_id]/page.jsx
...
export default function KnowledgeBaseDetailPage({ params }) {
  ...
  const { knowledgeBase, isLoading } = useKnowledgeBase(params.kb_id);
  const {
    documents,
    isLoading: documentsLoading,
    refresh: refreshDocuments,
  } = useDocuments(params.kb_id, { page_size: 50 });
  const { stats } = useKnowledgeBaseStats(params.kb_id);
  ...
  const handleUpload = async (files) => {
    ...
    try {
      for (const file of files) {
        ...
        await uploadDocument(params.kb_id, formData);
      }
  ...
  const handleDelete = async (document) => {
    try {
      await deleteDocument(params.kb_id, document.id);
  ...
  const handleReprocess = async (document) => {
    try {
      await reprocessDocument(params.kb_id, document.id);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex flex-col gap-2">
            <Button
              ...
              onClick={() => router.push(ROUTES.KNOWLEDGE_BASES)}
            >
            ...
            <div className="flex items-center gap-2">
              <Badge variant="muted">
                {VECTOR_STORE_LABELS[knowledgeBase?.vector_store] || knowledgeBase?.vector_store}
              </Badge>
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(ROUTES.KNOWLEDGE_BASE_CHUNKS(params.kb_id))}
            >
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(ROUTES.KNOWLEDGE_BASE_RETRIEVAL(params.kb_id))}
            >
                ...
                {documentsLoading ? (
                  ...
                  <div className="p-4">
                    <EmptyState
                      ...
                      description="Upload files from the Upload tab to start building this knowledge base."
                    />
                  </div>
                ) : (
                  <Table>
          ...
          <TabsContent value="upload">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Files will be chunked using the {knowledgeBase?.chunking_strategy} strategy and
                  embedded with {knowledgeBase?.embedding_provider}
                </CardDescription>
              </CardHeader>
              <CardContent>
