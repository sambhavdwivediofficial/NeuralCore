...
from sqlalchemy import Column, DateTime, Enum, ForeignKey, String, UniqueConstraint, delete, select
...
async def list_project_members(
    ...
    await _get_project_or_404(db, project_id, tenant_id)
    ...
    return [
        _member_response(m, users_map[m.user_id])
        for m in members
...
async def add_project_member(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_roles(Role.ADMIN, Role.OWNER, Role.SUPER_ADMIN)),
) -> dict:
    tenant_id = current_user.organization_id
    if not tenant_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No organization context")
