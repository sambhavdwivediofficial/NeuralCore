...
from api.dependencies import CurrentUser, PaginationParams, get_app_settings, get_db, get_redis
...
async def login(
    ...
    if user.hashed_password is None or not verify_password(request_body.password, user.hashed_password):
        ...
        if user.failed_login_attempts + 1 >= settings.auth.account_lockout.max_failed_attempts:
            lockout_until = datetime.now(timezone.utc) + timedelta(minutes=settings.auth.account_lockout.lockout_duration_minutes)
...
@router.get("/sessions")
async def list_sessions(user: CurrentUser, redis=Depends(get_redis)) -> list[dict[str, Any]]:
    return [
        {
            "id": "current",
...
@router.get("/api-keys")
async def list_api_keys(user: CurrentUser, db=Depends(get_db)) -> list[dict[str, Any]]:
    return []
